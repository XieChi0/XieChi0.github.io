...
> 无状态就是服务端不需要存储 JWT 令牌的信息，每次收到令牌后，直接通过密钥验证签名是否合法、载荷里的过期时间是否有效，就能确认用户身份，不用去查数据库或缓存。这样服务端不用维护用户的登录状态，多个服务器之间也不用同步状态，方便横向扩展。
...
> JWT 和 Redis 结合通常是为了弥补它 “无法即时作废” 的缺点。比如把签发的 JWT 存到 Redis 里，用户注销或权限变更时，就从 Redis 删除对应的 JWT，服务端验证时先查 Redis，如果令牌不存在就拒绝请求。
>
> 或者用 Redis 存 JWT 的黑名单，过期的、作废的令牌都放进去，验证时先检查是否在黑名单里。

只要结合了 Redis，服务端收到 JWT 后，都需要先去 Redis 查一下令牌是否在白名单或黑名单里，再验证签名和过期时间。这一步会增加一点性能开销，但相比纯 JWT 的 “无法即时作废”，对需要强状态控制的场景来说是必要的权衡。
...
如果服务器把 JWT 放在带 HttpOnly 和 Secure 的 Cookie 里，前端后续请求时浏览器会自动把这个 Cookie 加到请求头里，前端代码不用额外处理。

但如果存在 localStorage 或 sessionStorage，前端就需要手动在请求头里加 Authorization 字段。
...
要实现踢人和拉黑，结合 Redis、access_token 和 refresh_token 可以这样做：

- 踢人时，当用户 A 需要踢掉用户 B，服务端可以将用户 B 当前的 access_token 加入 Redis 黑名单，同时将用户 B 的 refresh_token 也失效，比如在 Redis 中删除对应的 refresh_token 记录。这样用户 B 的 access_token 很快过期，用 refresh_token 换新 access_token 时也会失败，被迫重新登录。
- 拉黑的话，服务端可以在 Redis 中维护一个 “拉黑用户 ID 列表”，当用户被拉黑后，无论他的 access_token 是否有效，服务端验证时先查这个列表，只要用户 ID 在黑名单里，直接拒绝所有请求，同时也让他的 refresh_token 失效。
