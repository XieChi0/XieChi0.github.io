...
> **Prisma 不是数据库！**
> **Prisma 不是数据库管理系统（DBMS）！**
> **Prisma 是一个 ORM 框架！**

### 常见误解

很多人看到 Prisma 的安装命令，以为在"安装数据库"：
...
这只是 **安装 Prisma 工具**，你的数据仍然存在 MySQL 或其他数据库里。
...
Prisma 本身**不存储数据**，它只是一个"翻译官"，把你的代码翻译成 SQL，然后发送给数据库执行。
...
1. **先学 SQL** — 理解 SQL 的原理和数据库的基本概念
2. **再学 ORM** — 理解 ORM 内部的工作机制
3. **两者结合** — 在复杂场景下知道何时用 SQL，何时用 ORM

> 理解 SQL 是学习 ORM 的基础，因为只有理解了 SQL 的原理，才能更好地理解 ORM 内部的工作机制。
