...
<div style="background-color: #52b1bf; color: white;">1. 地图如何初始化，谁负责持有 Cesium 实例。</div>
<div style="background-color: #52b1bf; color: white;">2. 业务页面如何调用地图能力，而不是每个页面都直接写 Cesium。</div>
<div style="background-color: #52b1bf; color: white;">3. 表格数据和地图实体如何互相定位、互相高亮。</div>
<div style="background-color: #52b1bf; color: white;">4. 多模型、多图层、多对象如何统一显示、隐藏、清除。</div>
<div style="background-color: #52b1bf; color: white;">5. 外部数据导入时，坐标系和数据质量如何保证。</div>
...
```
Earth (全局容器/单例)
├── viewer: Viewer实例  ← Cesium地图核心
├── imageryControl: ...  ← 影像图层管理
├── imageryMeasure: ...  ← 测量工具
├── imageryWatercourse: ...  ← 河道管理
└── ... 其他各种管理器
```
...
Earth (全局容器)
│
├── viewer ──────────────────→ Cesium 官方 API（原生类）
│
├── imageryControl ──────────→ 项目封装的业务代码（内部调用 viewer）
├── imageryMeasure ──────────→ 项目封装的业务代码（内部调用 viewer）
├── imageryWatercourse ──────→ 项目封装的业务代码（内部调用 viewer）
└── ...
```
...
> ```
>
> `new LayerWatercourse(viewer)` 是把 viewer 作为参数传进去，创建一个"河道管理器"实例，
>
> 这个管理器内部会持有这个 viewer，用来操作地图
...
```
本项目分层（按"功能模块"划分）：
├── LayerControl.ts      ← 通用控制（放大、缩小、复位）
├── LayerMeasure.ts      ← 通用测量（测距、测面）
├── LayerCommon.ts       ← 通用功能（画点、画线、坐标转换）
├── LayerImages.ts       ← 通用底图
│
├── LayerWatercourse.ts              ← 一维河道模块
├── LayerWaterlogTerrain.ts          ← 二维内涝地形模块
├── LayerWaterlogResultRendered.ts   ← 二维结果渲染模块
├── LayerOpeningWorks.ts             ← 闸泵站模块
└── ...
```
...
└─────────────────────────────────────────────────────────┘
                          ↓ 调用
┌─────────────────────────────────────────────────────────┐
│  业务封装层（MapLayer/index.vue）                        │ ← 你说的这层
│                                                          │
│  - 组合底层封装 + 业务逻辑                               │
│  - 对接菜单状态（currMenuParent）                        │
│  - 数据转换、store 同步、回调处理                        │
└─────────────────────────────────────────────────────────┘
                          ↓ 调用
┌─────────────────────────────────────────────────────────┐
...
| 业务逻辑     | 例子                                                   |
| :----------- | :----------------------------------------------------- |
| 菜单状态判断 | `if (currMenuParent !== 'OneDimensionalRiver') return` |
| 数据转换     | `genReach(params)` - 把 Cesium 数据转成业务模型        |
| Store 同步   | `oneDimenStore.setRiverSections(...)` - 同步到全局状态 |
| 回调处理     | 处理测量结果、渲染结果                                 |
| 多模块协调   | 同时操作多个封装模块                                   |
| UI 状态管理  | 显示/隐藏按钮、标签清理                                |
...
const layerRenderer = inject('layerRenderer')     
```
...
│                                                                     │
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐     │
│  │   <MapLayer />  │  │ <LayerRenderer> │  │   <Engineer />  │     │
│  │                 │  │                 │  │                 │     │
│  │  地图展示层       │  │  图层渲染器       │  │  业务页面容器     │     │
│  │  （Cesium）      │  │  （显隐控制）     │  │  （10+页面）      │     │
│  └─────────────────┘  └─────────────────┘  └─────────────────┘     │
...
**问：为什么在layer.ts 中已经创建全局 Earth 对象，并且用InitEarth 函数初始化 Earth 对象，**
...
└─────────┬─────────┘
          ...
          ▼
┌───────────────────┐
│   layerStore.ts   │  ← 状态管理（visibleLayers Set）
│                   │
│  visibleLayers    │
...
└─────────┬─────────┘
          ...
          ▼
┌───────────────────┐
│   views/index.vue │  ← 调度器（监听变化，分发给 renderer）
│                   │
│  watch visibleLayers
│    → newlyVisible
│    → newlyHidden
...
└─────────┬─────────┘
          ...
          ▼
┌───────────────────┐
│ LayerRenderer.vue  │  ← 渲染分发（display:none，不可见）
│                   │
│  showLayer(id)    │
│    → renderXxxLayer()
└─────────┬─────────┘
...
**设计原则**：图层体系遵循"关注点分离"原则，每一层只做自己该做的事：

```
┌─────────────────────────────────────────────────────────────────────┐
│  地图层（layerCore）                                                │
│  只负责：在 Cesium 上绘制、删除、高亮实体                           │
│  只发信号：click → emit('station:click', data)                     │
│  不关心：页面跳转、业务状态                                         │
└─────────────────────────────────────────────────────────────────────┘
                              ↓ 发信号
┌─────────────────────────────────────────────────────────────────────┐
│  业务层（业务页面 / MapLayer）                                      │
│  负责：收到信号后决定做什么                                         │
│  例如：调用 router.push() 跳转页面                                  │
│       调用 globalStore.setCurrPage() 更新状态                        │
└─────────────────────────────────────────────────────────────────────┘
```
...
│  LayerCommon.bindClickListener()                                  │
│    → viewer.scene.pick() 获取实体                                │
│    → 从 entity.properties 提取 type 和 data                     │
│    → emit 事件信号（不发决策）                                    │
│      例如：eventBus.emit('entity:station:click', { type, data }) │
└─────────────────────────────────────────────────────────────────┘
       │
       ▼ 发信号
┌─────────────────────────────────────────────────────────────────┐
│  业务页面监听                                                    │
│    eventBus.on('entity:station:click', (payload) => {           │
│      // 业务层决定：跳转到哪个页面                                │
│      router.push({ name: 'StationPage' })                       │
│      globalStore.setCurrPage({ id: 'station', ... })           │
│    })                                                            │
└─────────────────────────────────────────────────────────────────┘
```
...
> ⚠️ 当前代码中部分模块（如 `LayerWaterlogTerrain`、`LayerCommon`）为了快速上线，直接在地图层内部调用了 `globalStore.setCurrPage()` 和 `router.push()`，这是实现上的耦合问题。按照正确的架构，这些业务决策应该在业务层完成，地图层只负责发信号。

**理想的重构方向**：地图层去掉对 globalStore 和 router 的依赖，统一通过回调函数或事件总线发信号；业务层负责监听信号并执行页面跳转等副作用操作。
