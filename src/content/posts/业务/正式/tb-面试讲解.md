...
TBDock 是一个面向扩展坞硬件资料管理和对外展示的全栈平台，包含 Vue3 管理后台、Vue SSR 官网和 NestJS 服务端，主要解决硬件资料分散、参数复杂、录入效率低、官网首屏慢和 SEO 收录差的问题。

面试可以这样说：

> 这个项目不是一个简单的后台 CRUD，而是一个“硬件资料结构化管理 + 官网展示 + 性能和 SEO 优化”的完整平台。后台负责维护 500+ 扩展坞产品数据，官网负责把这些数据以对比表的形式展示给用户，服务端负责数据模型、鉴权、查询、上传、缓存刷新等能力。

### 0.2 项目为什么有技术含量

扩展坞这类硬件产品的难点在于：字段不是平铺的。

一个产品除了品牌、型号、图片，还会有：

- Thunderbolt、USB-C、HDMI、DP、RJ45、读卡器等端口信息。
- 供电能力、散热方式、系统兼容性。
- 视频输出能力，例如几路 4K 60Hz、是否支持 8K、色深、输出接口。
- 资料来源、价格、备注、维护状态。

所以项目主要有三个技术问题：

- 数据怎么建模和维护。
- 官网怎么让用户更快看到内容，并让搜索引擎抓到内容。
- 后台怎么降低复杂表单的开发和录入成本。
...
如果官网和后台混在一起，代码会越来越难维护。官网关注公开展示和 SEO，后台关注表单和管理效率，它们的目标不同，所以我把它们拆成两个 Vue 项目，再共用同一个 NestJS 服务端。

### 1.2 技术选型怎么讲

技术选型不是为了堆技术，而是围绕项目目标。

#### Vue3 + Vite

前后台都使用 Vue3 + Vite，原因是开发效率高，组件化和响应式适合大量表单、筛选项和页面状态管理。

#### Element Plus

后台使用 Element Plus，因为后台表单多、表格多、弹窗多，Element Plus 的表单、表格、选择器和校验能力比较适合管理系统。

#### Vue SSR

官网使用 SSR，是因为它是公开页面，需要 SEO 和首屏速度。普通 SPA 首屏 HTML 内容少，而 SSR 可以让服务端直接返回带产品内容的 HTML。

#### NestJS

服务端使用 NestJS，因为它模块化清晰，适合中后台 API 服务。项目里可以自然拆出 `DockModule`、`DictModule`、`AuthModule`、`CommonModule` 等模块。

#### Prisma + MySQL

扩展坞数据模型有 Product、Port、VideoCapability 等关联关系。Prisma 能把数据库模型和 TypeScript 类型结合起来，关系查询和事务处理都比较清晰。
...
这段代码在面试里非常好用，因为它能证明你不是“写了几个 controller”，而是把服务端公共能力先搭好了，后续官网和后台都复用这套基础设施。
...
1. 浏览器请求页面。
2. 服务端返回一个很空的 HTML，通常只有 `<div id="app"></div>`。
3. 浏览器下载 JS、CSS。
4. JS 执行后请求接口。
5. 接口返回数据。
6. Vue 在浏览器里生成页面内容。

问题是：用户刚打开页面时可能会先看到 loading；搜索引擎第一次抓 HTML 时，也可能看不到真实产品内容。
...
3. 服务端匹配路由。
4. 服务端预取页面所需数据。
5. 服务端把 Vue 组件渲染成 HTML 字符串。
6. 浏览器直接收到带内容的 HTML。
7. 客户端 JS 加载后接管这份 HTML，绑定事件和状态，这一步叫 hydrate。
...
const SSR_URLS = ['/']; // 只把最关键的首页放进 SSR 范围，避免把所有路由都变成服务端渲染负担
...
app.use(async (req, res, next) => {
  ...
  if (cached) {
    res.set({ 'X-SSR-Cache': 'HIT' }).end(cached); // 命中缓存时直接返回 HTML，首屏更快
    return;
  }

  const { html: appHtml, preloadedSsrData, piniaState } = await render(req.originalUrl);
  let html = template.replace('<!--ssr-outlet-->', appHtml); // 把 Vue 渲染结果塞回 HTML 模板
  ...
  res.set({ 'X-SSR-Cache': 'MISS' }).end(html);
});
```
...
export async function createApp(url: string) {
  ...
  const router = createRouter({
    history: createMemoryHistory(), // SSR 不能操作浏览器地址栏，所以要用 memory history
