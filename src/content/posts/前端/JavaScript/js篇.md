...
可以这么理解：JSON 整体是**文本格式**，它最外层只有两种合法结构：**JSON 对象**、**JSON 数组**，没有其他形式。

#### 1. JSON 对象（最外层是 `{}`）

语法：键名**必须双引号**，值支持字符串、数字、布尔、null、嵌套对象/数组。
...
数组里的每一项，可以是字符串、数字、对象、子数组等合法 JSON 数据。
...
1. **本质区分**
   - 我们口语说「JSON 对象/JSON 数组」，只是按**最外层括号**区分格式；
   - 它们本质都还是**字符串**，不是 JS 内存里真正的对象/数组。

2. **和 JS 对应关系**
   - JSON 对象字符串 → `JSON.parse()` 后得到 **JS 对象**
   - JSON 数组字符串 → `JSON.parse()` 后得到 **JS 数组**

#### 完整例子

```js
// 1. JSON 对象格式的字符串
let jsonObjStr = '{"name":"张三"}'
// 转成 JS 对象
let jsObj = JSON.parse(jsonObjStr)

// 2. JSON 数组格式的字符串
let jsonArrStr = '[1,2,{"a":3}]'
// 转成 JS 数组
let jsArr = JSON.parse(jsonArrStr)
```
