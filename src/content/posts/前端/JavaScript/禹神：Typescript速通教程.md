...
updated: 2026-07-12
description: 跟随禹神三小时速通教程整理的 TypeScript 入门笔记,覆盖类型系统、接口、泛型与编译配置。
...
## ⼆、为何需要 TypeScript
### 静态类型检查
- 在**代码运⾏前**进⾏检查,发现代码的错误或不合理之处,减⼩运⾏时出现异常的⼏率,此种检查叫『静态类型检查』,
...
**TS 会根据我们的代码,进⾏类型推断,例如下⾯代码中的变量 d ,只能存储数字**
...
> 在 JavaScript 中的这些内置构造函数: Number 、 String 、 Boolean ,⽤于创建对应的包装对象, 
>
> 在⽇常开发时**很少使⽤**,（比如let str2: String）
>
> 在 TypeScript 中也是同理,所以 在 TypeScript 中进⾏类型声明时,通常都是⽤⼩写的 number 、 string 、 boolean
>
> 
>
> **原始类型 VS 包装对象**
...
any 的含义是:任意类型,⼀旦将变量类型限制为 any ,那就意味着**放弃**了对该变量的类型检查。
...
unknown 可以理解为⼀个类型安全的 any，适⽤于: 起初不确定数据的具体类型,要后期才能确定 。
...
由于上面提到unknown不能赋值给别的类型，所以这里有几种解决方法，
...
```ts
// 以下这个函数没有返回值（throw不算返回），所以可以用never
...
void 的含义是空,即: 函数返回空, 调⽤者也不应依赖其返回值进⾏任何操作
...
那限制函数返回值时,是不是 undefined 和 void 就没区别呢?—— 有区别。因为还有这句话 :【返回值类型为 **void** 的函数,调⽤者不应依赖其返回值进⾏任何操作!】
...
**object** (⼩写)的含义是:所有⾮原始类型,可存储:对象、函数、数组等（不能存储原始类型）,由于限制的范围⽐较宽泛,在实际开发中使⽤的相对较少。
...
- 简单记忆:Object类型的变量可以存储 除了 undefined 和 null 的任何值。 
...
let person: { 
	name: string
	age?: number
	[key: string]: any // 索引签名（保证key是字符串类型即可）,key这个单词,换成其他的也可以(另一种常用写法：index)
...
备注:
- 这里的=> 并不是箭头函数的标志，这是ts中表示类型声明的东西。
- TypeScript 中的 => 左边是参数类型的约束，右边是返回类型的约束。 
- 函数类型声明还可以使⽤:接⼝、⾃定义类型等⽅式,下⽂中会详细讲解。 

#### 声明数组类型

```ts
let arr1: string[]  //数组中的每个元素是string类型
...
元组 (Tuple) 是⼀种特殊的数组类型，用于描述数组中元素的类型和顺序。

默认长度固定；如果使用可选元素（`?`）或剩余元素（`...`），则长度可以变，但已定义部分的顺序和类型仍然是固定的。
...
比如下面 判断逻辑的 up 、 down 、 left 、 right 是连续且相关的⼀组值,那此时就特别适合使⽤**枚举( enum )**。

#### 1.数字枚举

数字枚举⼀种最常⻅的枚举类型,其成员的值会⾃动递增,且数字枚举还具备**反向映射**的特点,在下⾯代码的打印中,不难发现:可以通过值来获取对应的枚举成员名称
...
console.log(Direction); // 打印Direction会看到如下内容（见下面注释）
...
```ts
const enum|Directions { //这里前面加了const
	Up,
	Down,
	Left,
	Right
...
| 修饰符    | 含义     | 具体规则                                        |
| --------- | -------- | ----------------------------------------------- |
| public    | 公开的   | 默认会被加上。可以被:类内部、子类、类外部访问。 |
...
- 概述:抽象类是⼀种⽆法被实例化的类（无法用new）,专⻔⽤来定义类的结构和⾏为，类中可以写抽象⽅法，也可以写具体实现。

  抽象类主要⽤来为其派⽣类提供⼀个基础结构,要求其派⽣类  必须实现其中的抽象⽅法。
...
> 我们定义⼀个抽象类 Package ,表示所有包裹的基本结构,
>
> 任何包裹都有重量属性 `weight` ,包裹都需要计算运费。
>
> 但不同类型的包裹(如:标准速度、特快专递)都有不同的运费计算⽅式,
>
> 因此⽤于计算运费的 calculate ⽅法是⼀个抽象⽅法,必须由具体的⼦类来实现。
...
> 总结:何时使⽤抽象类?
>
> 1. 提供基础实现：在抽象类中提供某些⽅法或为其提供基础实现,这样派⽣类就可以继承这些实现。
>
> 2. 确保关键实现：强制派⽣类实现⼀些关键⾏为。
>
> 3. 共享代码和逻辑:当多个类需要共享部分代码时,抽象类可以避免代码重复。

### 13. interface(接⼝)

interface 是⼀种`定义结构`的⽅式,主要作⽤是为:类、对象、函数等规定⼀种契约,这样，

可以确保代码的⼀致性和类型安全,

但要注意 interface `只能`定义格式，不能包含任何实现 !
...
interface PersonInterface {
    name: string;
    age: number;
    speak(n: number): void;	//方法，方法的参数和返回值。
}
...
![image-20260709204309305](/Users/xchi/Desktop/禹神：Typescript速通教程.assets/image-20260709204309305.png)

![image-20260709204328341](/Users/xchi/Desktop/禹神：Typescript速通教程.assets/image-20260709204328341.png)
...
泛型允许我们在定义函数、类或接⼝时，使⽤类型参数来表示**未指定的类型**，这些参数在具体使⽤时，才被指定具体的类型。
...
它的主要作⽤是为现有的Js代码提供类型信息，使得 TypeScript 能够在使用这些 Js库或模块时进行类型检查和提示。
...
// index.ts
// example.ts
import { add, mul } from "./demo.js";	//加入.d.ts，可以在ts文件中引入js，并有相应的类型注解
