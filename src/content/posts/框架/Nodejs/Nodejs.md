...
> 原文来源：Traversy Media - Node.js Crash Course  
> 翻译整理+补充：苯人
...
Node.js 使用**事件和回调**来处理这些操作，而不是阻塞代码执行。这让它能够同时处理数千个连接，这是 Node.js 快速高效的原因。
...
2. **任务分发：**
   - 同步任务：直接在 JS 主线程执行
   - 异步 I/O 任务：事件循环**注册回调函数**，将操作交给底层线程 / 系统内核处理
...
console.log(`32°C = ${celsiusToFahrenheit(32)}°F`)
```
...
> `-D` 是 `--save-dev` 的简写，意思是把包安装为 “开发依赖”，这些包只在开发阶段用到，比如代码检查工具、构建工具等。项目打包上线时，这些依赖不会被包含进去。对应的，如果不加 `-D` 也不加 `-g`，默认是安装为 “生产依赖”，是项目运行时必须的包，比如框架、库等。
>
> 在package.json中，`devDependencies` 字段里列的就是用 `-D` 安装的开发依赖，只有开发阶段需要。而生产依赖会放在 `dependencies` 字段，项目运行时必须依赖这些包。
>
> ![image-20260503130954078](./assets/image-20260503130954078.png)
...
![image-20260504160305223](./assets/image-20260504160305223.png)

**有中间件时，**

![image-20260504160441633](./assets/image-20260504160441633.png)
...
| 类型     | 作用                                                |
| -------- | --------------------------------------------------- |
| 中间件   | 处理通用逻辑，比如日志、权限、解析 JSON、设置响应头 |
| 处理函数 | 处理具体接口逻辑，比如查询用户、创建订单、删除商品  |
